...
/**
 * Web-compatible 3D viewer using standard HTML <iframe>.
 * Avoids react-native-webview completely on web.
 */
export const AnimalWebView = forwardRef<AnimalWebHandle, Props>(function AnimalWebView(
  ...
  const send = (type: string, payload?: object) => {
    try {
      ...
      if (win && win.__kpCmd) {
        win.__kpCmd({ type, ...payload });
      }
    } catch {
      /* ignore cross-origin / unmounted errors */
    }
  };
